...
export const CURRENT_LOCATION_ID = 'current-search';
...
const searchBounds = (latlng, distance) => {
  if (!latlng) {
    return null;
  }
...
const placeBounds = prediction => {
  if (prediction) {
    if (Array.isArray(prediction.bbox) && prediction.bbox.length === 4) {
      ...
      return searchBounds(placeOrigin(prediction), distance);
    }
  }
  return null;
};
    ...
    if (this.getPredictionId(prediction) === CURRENT_LOCATION_ID) {
      return userLocation().then(latlng => {
        return {
          address: '',
          origin: latlng,
          bounds: searchBounds(latlng, config.maps.search.currentSearchBoundsDistance),
        };
      });
    }
